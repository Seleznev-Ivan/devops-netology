...
| ------------- | ------------- | ------------- |
| `c`  | a+b  | Переменные a и b заданы в неявном виде и bash считает их как строковые числа |
| `d`  | 1+2  | Переменные a и b заданы со знаком $ и bash уже обращается к значениям переменных a и b |
| `e`  | 3  | Перед скобками ставится знак $ для того, чтобы bash выполнял конструкцию, которая содержит внутри, одни скобки экранируют лишние символы, а вторые скобки необходимы, чтобы преобразовать все значения переменных a и b в целые числа |
...
```
Во-первых не хватает закрывающей скобки в первой строке.

Во-вторых не хватает `else` в условии

В-третьих забыли указать оператор `break`, чтобы досрочно прервать цикл доступности сервиса (выход из цикла `for`, `while` или `until`)
