...
Что значит строчка "example_${random_password.random_string_FAKE.resulT}" я так и не понял.   
Задавал по данной ошибке вопросы в форме Задать вопрос, а также преподавателю данной лекции, но ответа не получил.

Поэтому поменял на значение name  = "terraform-nginx"
